...
`0.10.9`が表示され、UIからSystemを作成できることを確認します。Native Performanceログでは、初回評価後のGuide以外の編集で`guideSurface`のmissが0になり、Interactiveでは`meshSample=hit`、Directionだけの再編集では`neighborCache=hit`になることを確認します。
