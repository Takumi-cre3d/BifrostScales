...
if (Select-String -Path $nodeDefinition -Pattern "BifrostScales::BifrostScales" -Quiet) {
    throw (
        ...
        "BifrostScales::BifrostScales. Verify the 0.10.7 header source and rebuild with -Clean."
    )
}
try {
...
foreach ($requiredTopologyPortName in $requiredTopologyPortTypes.Keys) {
    ...
    if ($actualTopologyPortType -ne $expectedTopologyPortType) {
        throw (
            ...
            "Verify the 0.10.7 Native source and rebuild with -Clean."
        )
    }
}
...
if ([string]$metadataManifestData.native_behavior_contract -ne 'bifrost-scales/native-core/0.10.7-density-margin-curvature-surface-follow-1') {
    throw (
...
$moduleText = @"
+ BifrostScales 0.10.7 $modulePathText
PYTHONPATH +:= scripts
...
Write-Host ""
Write-Host "Bifrost Scales 0.10.7 uses Payload Schema 10 / Operator Contract 18 / Profile Schema 9, owner-safe Cell margins, curvature-following Cell interiors, process-shared Stage Cache, and OpenCL GPU Preview; do not reuse a pre-0.10.7 DLL." -ForegroundColor Yellow
