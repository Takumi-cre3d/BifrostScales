...
_INJECTED = weakref.WeakSet()

def _find_unique_scale_tabs():
    if QtWidgets is None: return []
    result = []
    app = QtWidgets.QApplication.instance()
    if app is None: return result
    for widget in app.allWidgets():
        if not isinstance(widget, QtWidgets.QTabWidget): continue
        for i in range(widget.count()):
            text = str(widget.tabText(i)).lower()
            if "unique" in text and "scale" in text or "ユニーク" in text:
                result.append(widget.widget(i))
    return result


def _inject_panel(tab) -> None:
    if tab in _INJECTED or QtWidgets is None: return
    layout = tab.layout()
    if layout is None:
        layout = QtWidgets.QVBoxLayout(tab)
    frame = QtWidgets.QFrame(tab); frame.setObjectName("bifrostScalesCellPickerPanel")
    frame.setFrameShape(QtWidgets.QFrame.Shape.StyledPanel if hasattr(QtWidgets.QFrame, "Shape") else QtWidgets.QFrame.StyledPanel)
    box = QtWidgets.QVBoxLayout(frame); box.setContentsMargins(6,6,6,6); box.setSpacing(4)
    row = QtWidgets.QHBoxLayout()
    toggle = QtWidgets.QPushButton("Select Cells", frame); toggle.setCheckable(True); toggle.setObjectName("bifrostScalesSelectCellsButton")
    rebuild = QtWidgets.QPushButton("Rebuild Pick Cache", frame)
    clear = QtWidgets.QPushButton("Clear Selection", frame)
    row.addWidget(toggle); row.addWidget(rebuild); row.addWidget(clear)
    status = QtWidgets.QLabel(frame); status.setWordWrap(True); status.setObjectName("bifrostScalesCellPickerStatus")
    help_label = QtWidgets.QLabel("Hover: highlight  |  Click: replace  |  Shift: add  |  Ctrl: remove  |  Esc: exit", frame)
    help_label.setWordWrap(True)
    box.addLayout(row); box.addWidget(status); box.addWidget(help_label)
    try: layout.insertWidget(0, frame)
    except Exception: layout.addWidget(frame)
    manager = get_manager()
    def refresh():
        status.setText(manager.status_text())
        if toggle.isChecked() != manager.enabled:
            toggle.blockSignals(True); toggle.setChecked(manager.enabled); toggle.blockSignals(False)
    def on_toggle(value):
        if value:
    ...
    rebuild.clicked.connect(manager.rebuild)
    clear.clicked.connect(manager.clear)
    manager.add_listener(refresh)
    refresh(); _INJECTED.add(tab)


def install_ui_injector() -> None:
    if not MAYA_AVAILABLE or QtCore is None: return
    def scan():
        for tab in _find_unique_scale_tabs(): _inject_panel(tab)
    QtCore.QTimer.singleShot(0, scan)
    timer = QtCore.QTimer(QtWidgets.QApplication.instance())
    timer.setInterval(1000); timer.timeout.connect(scan); timer.start()
    # Keep alive on QApplication.
    setattr(QtWidgets.QApplication.instance(), "_bifrost_scales_picker_timer", timer)
