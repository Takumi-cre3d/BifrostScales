...
MINIMUM_NATIVE_PACK_VERSION_TEXT = "0.10.7"
...
def probe_native_backend(
    ...
    if not pack_config.is_file():
        ...
        reasons.append(
            ...
            "Rebuild the 0.10.7 Native Pack with -Clean and completely restart Maya.".format(
        ...
        reasons.append(
            ...
            "rebuild the 0.10.7 Native Pack with -Clean and "
    ...
    if pack_graph is None:
        ...
        reasons.append(
            ...
            "source_mesh input wired to get_mesh_structure. Reinstall Bifrost Scales 0.10.7 or "
            "rebuild the compatible 0.10.7 Native Core Pack."
        )
    elif pack_graph_payload_schema != NATIVE_PAYLOAD_SCHEMA:
        rebuild_required = True
        reasons.append(
            "Native Pack graph payload schema is {} but Python requires {}. "
            "Reinstall Bifrost Scales 0.10.7; if the mismatch remains, rebuild the "
            "0.10.7 Native Pack with -Clean and restart Maya.".format(
    ...
    if pack_config.is_file() and pack_manifest_profile_schema != NATIVE_PROFILE_SCHEMA:
        ...
        reasons.append(
            ...
            "Rebuild the 0.10.7 Native Pack with -Clean.".format(
    ...
    if pack_config_active:
        if not catalog_query_available:
            ...
            reasons.append(
                ...
                "Reinstall Bifrost Scales 0.10.7, rebuild the Native Pack with -Clean, then "
            ...
            reasons.append(
                ...
                "contain the exact definition {}{}. Rebuild the compatible 0.10.7 core pack, then "
    ...
    def _validate_graph_contract(
        ...
        if (
            ...
            raise RuntimeError(
                ...
                "contract. Delete the Native Graph and create a new one with Bifrost Scales 0.10.7."
            )
    ...
    def _validate_graph_target(
        ...
        if (
            ...
            raise RuntimeError(
                ...
                "Delete the Native Graph and create it again with Bifrost Scales 0.10.7."
            )
